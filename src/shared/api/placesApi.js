...
function mapPlaceFromApi(place, images = []) {
...
export const placesApi = {
    async getMapPlaces(params = {}) {
        ...
        return {
            places: Array.isArray(data.places)
                ? data.places.map((place) => mapPlaceFromApi(place)).filter((place) => place.position)
                : [],
        };
    },
    ...
    async getPlaceBySlug(slug) {
        ...
        return {
            place: data.place ? mapPlaceFromApi(data.place, data.images ?? []) : null,
        };
    },
};